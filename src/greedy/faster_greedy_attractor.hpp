...
class FasterGreedyAttractor
{
    ...
    static std::pair<uint64_t, uint64_t> decrementFrequencies(LCPInterval<uint64_t> &removedInterval, std::vector<uint64_t> &countVec, std::unordered_map<uint64_t, std::set<uint64_t>> &freqRankMap, std::vector<uint64_t> &sa);
