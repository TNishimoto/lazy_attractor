...
for file in ${filearr[@]}; do
	echo "Lazy attractor : ${file}"
    nohup /usr/bin/time -f "# lazy ${file}, %e sec, %M KB" ~/lazy_attractor/build/lazy.out -i ${folderpath}/${file} -o ${outputpath}/${file}.lazy.attrs >> ~/output.txt 2>> ~/error.log
done